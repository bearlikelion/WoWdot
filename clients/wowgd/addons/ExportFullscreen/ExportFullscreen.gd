...
## Plugin that automatically sets window mode to borderless fullscreen during export,
...
# Matches the Settings default so the first window is not a mode the game
# immediately switches away from.
const WINDOW_MODE_FULLSCREEN: int = 3
...
class ExportFullscreenExporter extends EditorExportPlugin:
	...
	func _export_begin(features: PackedStringArray, is_debug: bool, path: String, flags: int) -> void:
		...
		if err != OK:
			...
			print("ExportFullscreen: Set window mode to borderless fullscreen for export (original: %d)" % _original_mode)
