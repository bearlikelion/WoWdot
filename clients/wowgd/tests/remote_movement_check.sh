#!/bin/sh
# Makes the partner's character on the second test account, runs the movement check, and deletes it again.
cd "$(dirname "$0")/.." || exit 1
...
godot --path . --resolution 1280x960 tests/remote_movement_check.tscn
status=$?
