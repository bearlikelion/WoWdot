...
func _update_latency() -> void:
	var latency: int = WowClient.session.get_latency()
	if latency > MEDIUM_LATENCY:
		_performance_bar.modulate = Color.RED
	elif latency > LOW_LATENCY:
