#!/usr/bin/env bash
# Builds the Linux release library in the same buildroot SDK image as the Godot export template (glibc 2.28 floor), then the Windows one with the host's mingw-w64.
set -Eeuo pipefail
...
(cd extension && scons -j"$(nproc)" platform=windows target=template_release)
