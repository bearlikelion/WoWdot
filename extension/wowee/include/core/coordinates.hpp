...
// Light.dbc positions, radii and fog distances are placement units scaled by 36.
inline constexpr float LIGHT_DBC_SCALE = 36.0f;

inline glm::vec3 lightDbcToWorld(float x, float height, float z) {
    return adtToWorld(x / LIGHT_DBC_SCALE, height / LIGHT_DBC_SCALE, z / LIGHT_DBC_SCALE);
}
